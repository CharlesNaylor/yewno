...
Now for the returns measure we're going to use. This sort of thing is much easier in `pandas`.
```{r weekly_returns}
y <- fut %>% filter(series %in% c("Open", "Last")) %>%
  ...
  summarize(Date=last(Date),
            change=log(last(Last)) - log(first(Open))) %>%
...
backtest %>% mutate(curve=cumsum(curve),
                    equal=cumsum(equal)) %>% #Cumulative sum since we did returns in log space
